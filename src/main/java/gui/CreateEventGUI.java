...
public class CreateEventGUI extends JFrame {
	...
	private void jbInit(Vector<domain.Event> v) throws Exception {
		...
		eventfield.setBounds(new Rectangle(100, 211, 429, 20));
		...
		jButtonCreate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(eventfield.getText().length() == 0) {
					...
					jop.showMessageDialog(jButtonCreate, "Idatzi gertaeraren deskripzio bat");
				}else {
					Event ev = new Event(kont, eventfield.getText(), jCalendar.getDate());
					if(facade.createEvent(ev)) {
						JOptionPane jop = new JOptionPane();
						jop.showMessageDialog(jButtonCreate, "Gertaera gorde da");
					}else {
						JOptionPane jop = new JOptionPane();
						jop.showMessageDialog(jButtonCreate, "Ez da gertaera sortu");
					}
				}
			}
		});
		...
		this.getContentPane().add(eventfield, null);
